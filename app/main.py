...
def get_today_article(user_word_list, articleID, user_freq_record):
    ...
    marks = load_freq_history(user_freq_record)
    if len(marks) > 0:
        lst = pickle_idea2.dict2lst(marks)
        lst2 = []
        for t in lst:
            lst2.append((t[0], len(t[1])))
        for x in lst2:
            print(x[0])
            article_title = article_title.replace(x[0],'<span class="highlight1">'+x[0]+'</span>')
            article_body = article_body.replace(x[0],'<span class="highlight1">'+x[0]+'</span>')
...
def userpage(username):
    ...
    if request.method == 'POST':  # when we submit a form
        ...
        page += '<div id="text-content">%s</div>'  % (get_today_article(user_freq_record, session['articleID'], user_freq_record))
                 ...
                 <script>
                   ...
                   function highlightWord(){
                       ...
                       if (word.trim() !== ""){
                           ...
                           var newWord = e.innerHTML.replaceAll(getWord(),'<span class="highlight">'+getWord()+'</span>')
                           e.innerHTML = newWord
                        }
                    }
